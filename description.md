...
**선물바구니 상세기능**(단건 선물하기와 다른 기능이다)
- 로그인한 회원만 선물바구니에 접근가능
- 회원가입시 회원마다 선물바구니가 하나씩존재한다.(GiftStatus가 Basket인 Gift)
- 선물바구니에 담기를 누르면 GiftStatus가 Basket인 Gift에 GiftItem이 하나씩 추가가 된다.
- 선물바구니에서 선물하기를 누르려면 받는 회원이 지정되어있어야한다.(receiveMember)
- 선물바구니에서 선물하기를 누르면 GiftStatus가 Created로 변경
- 선물바구니에서 비우기를 누르면 GiftItem이 모두 지워진다.
